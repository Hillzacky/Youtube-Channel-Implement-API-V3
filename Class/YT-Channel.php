...
class YTChannel {
                ...
                for ($x = 0; $x < $count; $x++) {
                    if (!empty($data->items[$x])) {
                        ...
                        if (!empty($data->items[$x]->replies->comments)) {
                            ...
                            //                                $result['comments'][$x]['replies'][$i]['author']['googleplus_url'] = (isset($data->items[$x]->snippet->topLevelComment->snippet->authorGoogleplusProfileUrl) ? $data->items[$x]->snippet->topLevelComment->snippet->authorGoogleplusProfileUrl : null);
    ...
    public function video_info($id) {
        ...
        if ($data) {
            ...
            $result['like_count'] = !empty($data->items[0]->statistics->likeCount) ? $data->items[0]->statistics->likeCount : '';
            $result['dislike_count'] = !empty($data->items[0]->statistics->dislikeCount) ? $data->items[0]->statistics->dislikeCount : '';
